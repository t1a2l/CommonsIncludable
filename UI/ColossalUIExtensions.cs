...
namespace Commons.UI
{
    public static class ColossalUIExtensions
    {
        ...
        public static Tuple<string, T>[] GetDropdownOptions<T>(string localeKey) where T : struct => [.. Enum.GetValues(typeof(T)).Cast<T>().Select(x => Tuple.New(Locale.Get(localeKey, x.ToString()), x))];
        public static Tuple<string, int>[] GetDropdownOptions(string[] localeKeys) => [.. localeKeys.Select((x, i) => Tuple.New(Locale.Get(x), i))];
        public static Tuple<string, int>[] GetDropdownOptionsUnlocalized(string[] values) => [.. values.Select((x, i) => Tuple.New(x, i))];
        public static Tuple<string, T>[] GetDropdownOptions<T>(this T[] options, string localeKey) where T : struct => [.. options.Select(x => Tuple.New(Locale.Get(localeKey, x.ToString()), x))];
